...
<html>
    <head>
        ...
        <style>
            ...
            .btn-sidebar-left:hover { background-color: #27ae60; }
            ...
            .sidebar-left.visible { transform: translateX(0); }
            ...
            .sidebar-content-wrapper { padding: 10px; width: 100%; }
            ...
            .equipo-table tr:hover { background-color: rgba(44, 62, 80, 0.3); }
            ...
            .stat-label { font-size: 0.9rem; color: #bdc3c7; }

            /* TARJETA BLANCA REDONDEADA POR VELOCÍMETRO */
            .gauge-card {
                position: relative;
                width: 100%;
                max-width: 275px;
                ...
                cursor: pointer;                 /* <-- clic en TODO el marco */
                box-sizing: border-box;
                overflow: hidden;                /* <-- asegura que TODO quede dentro del marco */
            }
            .gauge-card:hover {
            ...
            /* fuerza a plotly a respetar el ancho del contenedor */
            .gauge-card .dash-graph, .gauge-card .js-plotly-plot {
                width: 100% !important;
            }

            /* RESALTADO: borde delgado (25%) */
            ...
            .gauge-highlight { animation: gaugePulseThin 1s ease-in-out infinite; }
            ...
            /* NOMBRE DE EQUIPO CUANDO HAY NUEVA ALERTA */
            .equipo-highlight {
                margin-top: 2px;
                padding: 4px 8px;
                border-radius: 999px;
            ...
            @media (max-width: 768px) {
                ...
                .sidebar-open-right .main-content { margin-right: 0; width: 100%; }
                .sidebar-open-left.sidebar-open-right .main-content {
                    margin-left: 350px;
                    margin-right: 0;
                    width: calc(100% - 350px);
...
# Estado global
conteo_alertas = {barco: 0 for barco in BARCOS_ATUNEROS}
alertas_sin_barco = 0
debug_info = []
...
def extraer_nombre_barco_de_area(area):
...
def normalizar_nombre_barco(nombre):
    if not nombre:
        return None
    ...
    mapeo_especial = {
        'RICKY A': 'BP RICKY A', 'BP RICKY': 'BP RICKY A', 'RICK A': 'BP RICKY A',
        'MARIA D MAR': 'MARIA DEL MAR', 'MARIA D EL MAR': 'MARIA DEL MAR',
        'ELIZABETH': 'ELIZABETH F', 'ROSA': 'ROSA F', 'MILENA': 'MILENA A',
        'MILAGROS': 'MILAGROS A', 'GLORIA': 'GLORIA A', 'ROBERTO': 'ROBERTO A',
        'GABRIELA': 'GABRIELA A', 'RAFA': 'RAFA A',
    }
...
def preparar_df_flota_24h(df_raw_local):
    debug = []
...
def procesar_alertas_ultimas_24h(df_raw_local):
...
def crear_velocimetro_24h(valor, titulo_barco, max_valor=30):
    if valor == 0:
        estado, color, emoji = "Sin Alerta A2", "#2ecc71", "✅"
    elif valor <= 6:
        estado, color, emoji = "Alerta", "#f1c40f", "⚠️"
    elif valor <= 10:
        estado, color, emoji = "Crítico", "#e74c3c", "🔴"
    else:
        estado, color, emoji = "Crítico al Maximo", "#c0392b", "🚨"

    rangos = [0, 0.5, 6.5, 10.5, max_valor]
    colores = [COLORES_FRANJAS['verde'], COLORES_FRANJAS['amarillo'],
               COLORES_FRANJAS['rojo_claro'], COLORES_FRANJAS['rojo_oscuro']]
    ...
    fig = go.Figure(go.Indicator(
        ...
        domain={'x': [0, 1], 'y': [0, 1]},
        gauge={
            'axis': {'range': [0, max_valor], 'tickwidth': 2, 'tickcolor': "#ffffff",
                     'tickfont': {'color': "#ffffff", 'size': 9}},
            'bar': {'color': "#000000", 'thickness': 0.8},
            'bgcolor': "rgba(10,10,10,0.3)",
            'borderwidth': 2,
            'bordercolor': "#7f8c8d",
            'steps': [{'range': [rangos[i], rangos[i+1]], 'color': colores[i]} for i in range(len(colores))],
            'threshold': {'line': {'color': "#000000", 'width': 3}, 'thickness': 0.8, 'value': valor}
        }
    ))

    # Ajustado para que el gráfico “entre” dentro de la tarjeta sin desbordar
    fig.update_layout(
        height=176,  # un poco menor para que quepa con título + padding
...
def obtener_detalle_barco_24h(df_raw_local, barco_seleccionado):
    if df_raw_local is None or df_raw_local.empty:
        return pd.DataFrame()
...
def crear_grafico_barras_apilado(df_detalle, barco_seleccionado):
    if df_detalle.empty:
        fig = go.Figure()
        fig.add_annotation(
    ...
    fig.update_layout(
        title={'text': f'Alertas por Equipo - {barco_seleccionado} (Últimas 24 horas)',
...
def crear_tabla_equipos_detallada(df_detalle):
    if df_detalle.empty:
        return html.Div([
            html.P("No hay datos detallados disponibles para las últimas 24 horas.",
    ...
    for equipo in equipos_unicos:
        ...
        filas.append(html.Tr([
            ...
            html.Td(total_equipo, style={'textAlign': 'center', 'fontWeight': 'bold', 'color': '#2ecc71'}),
            html.Td(html.Div(badges, style={'display': 'flex', 'flexWrap': 'wrap', 'gap': '5px'}))
        ]))

    return html.Div([
        html.H6("Detalle por Equipo (Últimas 24 horas)", style={'color': '#2ecc71', 'marginBottom': '15px'}),
        html.Table([
            html.Thead(
                html.Tr([
...
def obtener_equipo_mas_reciente_por_barco(df_raw_local, barco):
    try:
        if df_raw_local is None or df_raw_local.empty:
            return None
...
# =========================
app.layout = html.Div([
    html.Button("⚙️", id="toggle-sidebar-left", className="btn-sidebar-left", title="Mostrar/Ocultar Configuración"),

    html.Button("×", id="close-sidebar-right", className="close-sidebar-btn", style={'display': 'none'}),

    html.Div(id="sidebar-overlay", className="overlay"),

    # Sidebar izquierda
    html.Div([
        html.H4("⚙️ CONFIGURACIÓN", style={'color': '#2ecc71', 'marginBottom': '20px'}),
        html.Hr(style={'borderColor': '#2c3e50'}),

        html.Label("⏱️ Intervalo de actualización (segundos):", style={'color': '#ecf0f1', 'marginTop': '15px'}),
        dcc.Slider(
            id='intervalo-slider',
        ...
        html.Br(),
        dbc.Button("🔄 ACTUALIZAR AHORA", id="btn-actualizar", color="primary", className="btn-primary",
    ...
    html.Div([
        html.Div([
            html.H4("🚢 DETALLES DEL BARCO", style={'color': '#2ecc71', 'marginBottom': '20px', 'fontSize': '22px'}),
    ...
    html.Div([
        dbc.Container([
            dbc.Row([
                dbc.Col([
                    ...
                    html.H5(
                        ...
                        style={'textAlign': 'center', 'color': '#ecf0f1', 'marginBottom': '30px'}
                    )
                ], width=12)
            ]),
            ...
            html.Hr(style={'borderColor': '#2c3e50', 'marginTop': '30px', 'marginBottom': '20px'}),
            html.H4("📈 ESTADÍSTICAS GENERALES", style={'color': '#2ecc71', 'marginBottom': '20px'}),

            dbc.Row([
                dbc.Col([
                    html.Div([html.Div(id="total-alertas", className="metric-value"),
                              html.Div("Total Alertas Flota", className="metric-label")], className="metric-card")
                ], width=3),
                dbc.Col([
                    html.Div([html.Div(id="barcos-con-alertas", className="metric-value"),
                              html.Div("Barcos con Alertas", className="metric-label")], className="metric-card")
                ], width=3),
                dbc.Col([
                    html.Div([html.Div(id="estado-critico", className="metric-value"),
                              html.Div("Estado Crítico", className="metric-label")], className="metric-card")
                ], width=3),
                dbc.Col([
                    html.Div([html.Div(id="alertas-sin-barco", className="metric-value"),
                              html.Div("Alertas Sin Barco", className="metric-label")], className="metric-card")
                ], width=3),
            ]),

            dbc.Alert(id="nota-alertas-sin-barco", color="warning", dismissable=True, is_open=False,
                      style={'marginTop': '20px'}),
            ...
            dcc.Interval(id='highlight-timer', interval=1000, n_intervals=0),

            dcc.Store(id='alertas-data', data={'conteo_alertas': {}, 'alertas_sin_barco': 0}),
            dcc.Store(id='ultima-actualizacion', data=datetime.now().isoformat()),
            dcc.Store(id='sidebar-left-state', data={'visible': False}),
            dcc.Store(id='sidebar-right-state', data={'visible': False}),
            dcc.Store(id='raw-data-store'),
            dcc.Store(id='selected-boat', data=None),

            dcc.Store(id='prev-alertas-store', data={}),
            dcc.Store(id='highlight-store', data={'boats': [], 'until': None, 'equipos': {}}),
...
def actualizar_datos(n_clicks, n_intervals, intervalo, alertas_data_actual):
    global conteo_alertas, alertas_sin_barco, debug_info, ultima_actualizacion, df_raw
...
def actualizar_info_actualizacion(ultima_act_data, intervalo, _tick):
    if ultima_act_data:
        try:
            ...
            return html.Div([
                html.Div([
                    html.Span("🕒 Última actualización: ", style={'color': '#2ecc71', 'fontWeight': 'bold'}),
                    html.Span(ultima_act.strftime('%H:%M:%S'), style={'color': '#ecf0f1'})
                ]),
                html.Div([
                    html.Span("⏱️ Próxima: ", style={'color': '#2ecc71', 'fontWeight': 'bold'}),
                    html.Span(f"{tiempo_restante}s", style={'color': '#ecf0f1'})
                ])
            ])
        except:
            pass
...
def actualizar_velocimetros(alertas_data, n_intervals, highlight_data, n_ticks):
    ...
    for fila in range(3):
        cols = []
        inicio = fila * 5
        fin = min(inicio + 5, len(BARCOS_ATUNEROS))

        for barco in BARCOS_ATUNEROS[inicio:fin]:
            ...
            # IMPORTANTE:
            # - id + n_clicks en el contenedor (tarjeta) para que CLIC EN TODO EL MARCO abra sidebar derecha
            cols.append(
                dbc.Col([
                    html.Div(
                        [
                            ...
                            html.Div(
                                ...
                                style={'display': 'block' if (is_highlight and equipo_alerta) else 'none'}
                            ),
                            html.Div(
                                dcc.Graph(
                                    figure=fig,
                                    config={'displayModeBar': False},
                                    style={'width': '100%', 'height': '176px'}  # <-- dentro del marco
                                ),
                                style={'width': '100%'}
                            )
                        ],
        ...
        rows.append(dbc.Row(cols, className="gauge-row mb-5 g-4", justify='center'))
...
# Sidebar derecha:
# - ahora se abre con clic en TODO el marco (barco-card)
@app.callback(
...
def toggle_sidebar_right(card_clicks, close_clicks, overlay_clicks, raw_data, selected_boat, left_state, right_state):
    ...
    if trigger_id.startswith('{'):
        ...
        try:
            ...
            if df_detalle.empty:
                contenido = html.Div([
                    html.H5(f"{barco_seleccionado}", style={'color': '#2ecc71', 'marginBottom': '15px', 'fontSize': '20px'}),
                    html.P(f"No hay alertas registradas para {barco_seleccionado} en las últimas 24 horas.",
                           style={'color': '#bdc3c7', 'textAlign': 'center', 'padding': '30px', 'fontSize': '16px'})
                ])
            else:
                ...
                contenido = html.Div([
                    html.H5(f"{barco_seleccionado}", style={'color': '#2ecc71', 'marginBottom': '20px', 'fontSize': '24px', 'textAlign': 'center'}),
                    html.Div([
                        html.H6("Resumen de Alertas (Últimas 24 horas)", style={'color': '#2ecc71', 'marginBottom': '15px', 'fontSize': '18px'}),
                        html.Div([
                            html.Div([html.Div(str(total_alertas), className="stat-value"), html.Div("Total Alertas", className="stat-label")], className="stat-item"),
                            html.Div([html.Div(str(equipos_afectados), className="stat-value"), html.Div("Equipos Afectados", className="stat-label")], className="stat-item"),
                            html.Div([html.Div(str(tipos_alerta), className="stat-value"), html.Div("Tipos de Alerta", className="stat-label")], className="stat-item"),
            ...
            return ("sidebar sidebar-right visible",
                    contenido,
                    {'display': 'block'},
                    barco_seleccionado,
                    {'visible': True},
    ...
    # Cerrar derecha (botón o clic en overlay)
